...
export class RecibidosComponent implements OnInit {
   ...
   * @param _idEmpresa The ID of the company (consumer).
   * @param _fechaDesde The start date for the document search.
   * @param _fechaHasta The end date for the document search.
   */
  getDocumentReceivedConsumerList(_idEmpresa: number, _fechaDesde: string, _fechaHasta: string) {
    this.documentoService.getDocumentReceivedConsumer(_idEmpresa, _fechaDesde, _fechaHasta)
      .subscribe(ok => {
  ...
  /**
   * Initializes the component and retrieves the list of received documents for the consumer.
   */
  ngOnInit(): void {
    console.log("ngOnInit APLICATION DOC RECIBIDOS CONSUMIDOR");
    this.getDocumentReceivedConsumerList(83, "2023-01-01", "2023-12-31");
  }
