...
@Component({
  selector: 'app-reporte-emitidos-por-anio',
  templateUrl: './reporte-emitidos-por-anio.component.html',
  styleUrls: ['./reporte-emitidos-por-anio.component.css']
})
export class ReporteEmitidosPorAnioComponent {
  ...
  getReportEmitidos(idEmpresa: number): void {
    this.loading = true;
    this.reporteService.getReportDocumentsByTypeAndBusiness('issued', idEmpresa).subscribe(response => {
  ...
  exportReportPDF(): void {
    const originalElement = document.querySelector('app-reporte-emitidos-por-anio');
    ...
    if (originalElement instanceof HTMLElement) {
      ...
      titleElement.textContent = `Reporte Documentos Emitidos por Año - ${this.selectedBusinessName ?? ''}`;
      clonedElement.insertBefore(titleElement, clonedElement.firstChild);

      const options = {
        margin: 0.8,
        filename: `ReporteDocEmitidos_${this.selectedBusinessName ?? ''}.pdf`,
